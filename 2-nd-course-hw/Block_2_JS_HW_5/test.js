...
const littleNum = function () {
    ...
    return num1 < num2 ? alert(`Меньшее из двух чисел - число "${num1}"`) : alert(`Меньшее из двух чисел - число "${num2}"`);
}
littleNum();
...
const checkNumber = () => {
    let number = Number(prompt("Введите число."));
     if (isNaN(number)) {
        ...
        return alert(`${number} в кубе равняется ${mathOperation}`); //напиши плиз как тут без алерта вывести ретерн, а то не натыкался перебором )))
        // console.log(`${number} в кубе равняется ${mathOperation}`);
     }
}
checkNumber();
...
const cicle1 = {
    radius: 4,
    getArea() { 
        let mathRadius = Math.PI * (this.radius ** 2);
         return mathRadius; 
        },
    getPerimeter() {
        let mathPerimeter = (2 * Math.PI) * this.radius;
